...
@Slf4j
@Component
@RabbitListener(queuesToDeclare = {@Queue("short_link.delete.link.queue")})
public class ShortLinkDelLinkMQListener {
    ...
    public void shortLinkHandler(EventMessage eventMessage, Message message, Channel channel) throws IOException {
        log.info("C 端监听到消息-删除短链-消息内容：{}",message);

        try {
            eventMessage.setEventMessageType(EventMessageType.SHORT_LINK_DEL_LINK.name());

            // 处理消息
            boolean result = shortLinkService.handlerDelShortLink(eventMessage);

        } catch (Exception e) {
            log.error("C 端-删除短链-消费异常：{}", e.getMessage());
            throw new BizException(BizCodeEnum.MQ_CONSUME_EXCEPTION);
        }
        log.info("C 端-删除短链-消费成功{}", eventMessage);
    }
}
